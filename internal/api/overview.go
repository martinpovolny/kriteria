...
// overviewHandler serves a standalone HTML page that fetches /api/criteria
// and renders an expandable tree: grade → subject → criterion → levels.
// It uses vanilla JS + CSS, no build step needed.
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .status-error { padding: 40px; text-align: center; color: #c00; }
</style>
</head>
<body>
...
<div id="tree" class="tree">
  <div class="status-loading">Načítám data…</div>
</div>
...
async function load() {
  try {
    ...
    document.getElementById('tree').innerHTML =
      '<div class="status-error">Chyba při načítání: ' + e.message + '</div>';
  }
}
...
function render() {
  ...
  // Group by grade across all subjects: grade → [subject]
  const gradesMap = {};
  for (const subj of subjects) {
    for (const grade of subj.grades || []) {
      const key = grade.level;
      if (!gradesMap[key]) gradesMap[key] = [];
      gradesMap[key].push({ subject: subj, grade: grade });
    }
  }
    ...
    for (const entry of entries) {
      ...
      for (const crit of criteria) {
        ...
        if (hasLevels) {
          ...
          // Levels are already sorted Ú(4) first
...
function expandMissing() {
  ...
  collapseAll();
  // Then expand grade → subject for any subject that has missing levels
  document.querySelectorAll('.node').forEach(function(node) {
    const badges = node.querySelectorAll('.badge.missing');
    if (badges.length > 0) {
      // Expand this node's row if it has children
